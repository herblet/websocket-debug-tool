...
class WebSocketDebugTool extends React.PureComponent {
    ...
    try {
      ...
      if (this.state.stomp) {
        ...
        client.onStompError = (frame) => {
          ...
          that.info(
            `Connect STOMP server error, url = ${that.state.url}, connectHeader = ${this.state.stompConnectHeader}`
          );
        }
        ...
        client.onWebSocketError = (ev) => {
          that.setState({ connected: false });
          that.info(
            `Connect Websocket server error, url = ${that.state.url}, connectHeader = ${this.state.stompConnectHeader}`
          );
        }
        client.activate();
      } else {
        ...
        client.onerror = (e) => {
          ...
          that.error(
            `Connect error, message = ${e.data}, view chrome console for detail`
          );
          that.setState({ connected: false });
        };
      }
      ...
      console.error("Connect error %o", e);
      that.error(
        `Connect error, message = ${e.message}, view chrome console for detail`
      );
      that.setState({ connected: false });
      return;
    }
      ...
      if (this.state.stomp) {
        this.client
          ...
          .catch(() => {
            ...
            this.error(
              `disconnect fail, message = ${e.message}, view chrome console for detail`
            );
          });
      } else {
        this.client.close();
  ...
  send = () => {
    try {
      if (this.state.stomp) {
        ...
        if (this.state.stompSendHeader.length !== 0) {
          try {
            ...
            this.error(
              `STOMP send header format error: ${this.state.stompSendHeader}`
            );
            return;
          }
        }
        ...
        this.client.publish({
          destination: this.state.stompSendDestination,
          headers: headerJSON,
      ...
      this.error(
        `send message fail, message = ${e.message}, view chrome console for detail`
      );
    }
  };
  ...
  subscribe = () => {
    ...
    try {
      this.client.subscribe(
        ...
        this.getSubscribeCallback(this.state.stompSubscribeDestination)
      );
      this.info(
        `subscribe destination ${this.state.stompSubscribeDestination} success`
      );
    } catch (e) {
      console.error("subscribe fail: %o", e);
      this.error(
        `subscribe destination ${this.state.stompSubscribeDestination} fail, message = ${e.message}`
      );
    }
  };
  ...
  getSubscribeCallback(destination) {
    return (content) => {
      this.info(
        `Receive subscribed message from destination ${destination}, content = ${content.body}`
      );
    };
  }
          ...
          <Form inline>
            <FormItem label="Connect Type">
              <CheckboxGroup
                ...
                value={connectTypeArray}
                onChange={this.handleConnectTypeChange}
                disabled={this.state.connected}
              />
            </FormItem>
          </Form>
        ...
        <div className="stomp">
          ...
          <Form inline style={{ marginTop: "5px" }}>
            <FormItem label="STOMP connect header">
              <Input
                placeholder='json string, e.g. {"header1":"value1", "header2":"value2"}'
                style={{ width: "400px" }}
                value={this.state.stompConnectHeader}
                onChange={this.handleStompConnectHeaderChange}
          ...
          <Form inline style={{ marginTop: "5px" }}>
            <FormItem label="STOMP subscribe destination">
              <Input
                placeholder="e.g. /topic/test"
                style={{ width: "230px" }}
                value={this.state.stompSubscribeDestination}
          ...
          <Form inline style={{ marginTop: "5px" }}>
            <FormItem label="STOMP send header">
              <Input
                placeholder='json string, e.g. {"header1":"value1", "header2":"value2"}'
                style={{ width: "400px" }}
                value={this.state.stompSendHeader}
                onChange={this.handleStompSendHeaderChange}
                disabled={!(this.state.stomp && this.state.connected)}
              />
            </FormItem>
          </Form>
          <Form inline style={{ marginTop: "5px" }}>
            <FormItem label="STOMP send destination">
              <Input
                placeholder="e.g. /app/test"
                style={{ width: "400px" }}
                value={this.state.stompSendDestination}
        ...
        <div className="message">
          <Form inline>
            <FormItem label="Message Content">
              <Input
                placeholder="message content sent to server"
                style={{ width: "290px" }}
            ...
            <FormItem>
              <Button type="primary" onClick={this.clearOutput}>
                Clear Output
              </Button>
            </FormItem>
          </Form>
